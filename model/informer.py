...
class Informer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_enc = x[:, :, :-6]
        x_mark_enc = x[:, :, -6:]
        x_dec = x[:, -self.label_len:, :-6]
        x_mark_dec = x[:, -self.label_1en:, -6:]
        # don't need to transpose here
        enc_out = self.enc_embedding(x_enc, x_mark_enc)
        enc_out, = self.encoder(enc_out)
        if self.decode_method == 'linear':
            enc_out, = self.rnn(enc_out)
            output = self.projection(enc_out[:, -1, :])
        else:
            dec_out = self.dec_embedding(x_dec, x_mark_dec)
            # dec_out.shape: [1@24, 8, 256]
            dec_out = self.decoder(dec_out, enc_out)
            # enc_out.shape:[1024, 45, 256]
