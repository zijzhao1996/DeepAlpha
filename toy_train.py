...
import torch
import numpy as np
from utils.trainer import pytorch_train_toy, lightgbm_train, xgboost_train, mtl_train_toy
from utils.data import load_toy_train_data, load_tree_train_data
...
if __name__== '__main__':
    ...
    parser.add_argument('-c', '--config', default='./config/LSTM/phase1/LSTM_01.yaml', type=str,
    ...
    # get logger
    logger = get_logger( 'info', os.path.join(
    config['training']['log_save_path'], 'train_log.txt'))
    logger.info('######################################')
    ...
    if config['system']['ispytorch']:
        ...
        logger.info('Total number of parameters is {:.2f}KB ({:.2f}MB).'.format(count_parameters(model,
        "k"), count_parameters(model, "m")))
        logger.info('Constructing training model successfully.')
        train_loader, valid_loader = load_toy_train_data(config) 
        logger.info('Loading training data successfully.')
    
    if config[ 'system'][ 'model_name'] in ['MTL', 'MTLformer']:
        mtl_train_toy(train_loader, valid_loader, model, config, logger)
    else: 
        pytorch_train_toy(train_loader, valid_loader, model, config, logger)
    if config[ 'system'][ 'istree']:
        X_train, y_train, X_valid, y_valid = load_tree_train_data(config)
        logger.info('Loading training data successfully.')
        if config[ 'system']['model_name'] == 'LightGBM':
            ########## LightGBM model ##############
            model = lightgbm_train(X_train, y_train, X_valid, y_valid, config, logger)
